...
export default function RoomsPage() {
  ...
  const [rooms, setRooms] = useState<any>({ rooms: [], total: 0 }); // Initialize with matching shape
  ...
  const getStatusBadge = (room: any) => {
    if (!room.is_active) {
        return <span className="flex items-center gap-1 bg-red-100 text-red-700 px-2 py-1 rounded-full text-xs"><XCircle size={12}/> Inactiva</span>;
    }
    if (room.verification_status === "verified") {
        return <span className="flex items-center gap-1 bg-blue-100 text-blue-700 px-2 py-1 rounded-full text-xs"><CheckCircle size={12}/> Verificada</span>;
    }
    if (room.verification_status === "managed") {
        return <span className="flex items-center gap-1 bg-purple-100 text-purple-700 px-2 py-1 rounded-full text-xs"><CheckCircle size={12}/> Gestionada</span>;
    }
    return <span className="flex items-center gap-1 bg-yellow-100 text-yellow-800 px-2 py-1 rounded-full text-xs"><AlertCircle size={12}/> Sin Verificar</span>;
  };
