...
import { ArrowLeft, Building2, Code } from "lucide-react";

function CheckCircleIcon({ status }: { status: string }) {
  if (status === "verified") return <div className="w-5 h-5 rounded-full bg-blue-500" />;
  if (status === "managed") return <div className="w-5 h-5 rounded-full bg-purple-500" />;
...
export default function CreateRoomPage() {
  ...
  useEffect(() => {
    admin.getOrganizations().then(setOrgs).catch(() => {});
  }, []);
      ...
      <form onSubmit={handleSubmit} className="max-w-4xl space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Si no seleccionas una organización, el juego será &apos;unverified&apos; o &apos;standalone&apos;.
                </p>
              </div>
            </div>
          </div>
        ...
        <Card className="p-6 border-t-4 border-[#1F6357]">
          ...
          <p className="text-xs text-gray-500 mb-4">
            Pega aquí el JSON completo del scraper. Estos datos se usarán para la ficha detallada.
          </p>
          <div className="relative">
            <textarea
              ...
              className={`w-full font-mono text-xs h-96 rounded-lg border p-4 focus:ring-2 outline-none ${
                jsonError ? "border-red-500 focus:ring-red-200" : "border-gray-300 focus:ring-[#1F6357]"
              }`}
            />
            {jsonError && (
        ...
        <div className="flex justify-end pt-4 pb-12">
          <Button
            ...
            className="bg-[#1F6357] hover:bg-[#164a41] text-white px-8 py-3 h-12 text-lg"
          >
